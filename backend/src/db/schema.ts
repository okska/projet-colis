...
export const driverProfiles = app.table('driver_profiles', {
  ...
  documents: jsonb('documents').$type<Record<string, unknown>>().default({}).notNull(),
...
export const listings = app.table(
  ...
  (table) => ({
    parcelDetailsHasRequiredFields: check(
      'listings_parcel_details_keys',
      sql`${table.parcelDetails} ?& array['weight_kg','length_cm','width_cm','height_cm']`,
    ),
...
export const listingDeliveryRequests = app.table(
  'listing_delivery_requests',
  {
    id: uuid('id').defaultRandom().primaryKey(),
    listingId: uuid('listing_id')
      .notNull()
  ...
  (table) => ({
    listingDriverUnique: uniqueIndex('listing_delivery_requests_listing_driver_key').on(
      table.listingId,
      table.driverId,
    ),
...
export const deliveryReviews = app.table(
  'delivery_reviews',
  {
    id: bigserial('id', { mode: 'number' }).primaryKey(),
...
export const listingWatchers = app.table(
  'listing_watchers',
  {
    listingId: uuid('listing_id')
      .notNull()
